...
!!! tip "AM 小结"
    ...
    - AM 的频谱由<span style="border-bottom: 1.5px dashed orange;">载频分量、上边带和下边带</span>组成。
...
有<span style="border-bottom: 1.5px dashed orange;">滤波法、相移法</span>俩种方法
...
在 $\pm\;\omega_{c}$  处必须具有互补对称(奇对称)特性，即<span style="border-bottom: 1.5px dashed orange;">一个边带的损失恰能被另一个边带补偿</span>
...
其中，<span style="border-bottom: 1.5px dashed orange;">窄带噪声</span>可表示为同向分量与正交分量，有
...
!!! question "当 SNR 很小时"
    ...
    容易得到，信号被噪声所湮没以致完全不可用。这一现象称为<span style="border-bottom: 1.5px dashed orange;">门限效应</span>
