...
贝塞尔函数控制，<span style="border-bottom: 1.5px dashed orange;">调制前后总功率不变而重新分配</span>，与 `mf` 有关
...
> 因鉴频器的非线性特征，小 SNR 时也会出现<span style="border-bottom: 1.5px dashed orange;">门限效应</span>
...
!!! example "例 | 让 LLM 全军覆没的俩道题"
    ...
    > 需注意
    >
    > - 解调器输出端的信号已经<span style="border-bottom: 1.5px dashed orange;">搬回零频，重新考虑带宽，不含负频</span>
    > - SSB 的中频与载频不相等
