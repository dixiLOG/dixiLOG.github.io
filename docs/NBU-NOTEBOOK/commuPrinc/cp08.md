...
基于<span style="border-bottom: 1.5px dashed orange;">幅值、频率、相位</span>的载波参数控制，可分为<span style="border-bottom: 1.5px dashed orange;">ASK、FSK、PSK</span>
...
!!! info "总体可分为<span style="border-bottom: 1.5px dashed orange;">三板斧</span>"

    ![](https://cdn.jsdelivr.net/gh/dixiLOG/blogStatic/ThC3bnlOWofflExK5UxcIp3PnGf.png)

!!! tip "写在前面"

    接下去你可能会看见一个<span style="border-bottom: 1.5px dashed orange;">码元宽度</span>含 1 个、2 个、...载波周期
...
即<span style="border-bottom: 1.5px dashed orange;">带通滤噪</span>后<span style="border-bottom: 1.5px dashed orange;">全波整流</span>，再经过<span style="border-bottom: 1.5px dashed orange;">低通滤波</span>，最后<span style="border-bottom: 1.5px dashed orange;">抽样判决</span>
...
本振需要<span style="border-bottom: 1.5px dashed orange;">同频同相</span>
...
!!! quote "2PSK 存在载波锁相时<span style="border-bottom: 1.5px dashed orange;">倒 π</span>的问题(这在无差错检测位下无法得知)，于是有了 DPSK"

### 二进制差分相移键控 （2DPSK）

利用相邻的<span style="border-bottom: 1.5px dashed orange;">载波相位差</span>表示信息
...
多了个<span style="border-bottom: 1.5px dashed orange;">码变换</span>
...
> 当间距为带宽时，是<span style="border-bottom: 1.5px dashed orange;">单峰</span>与<span style="border-bottom: 1.5px dashed orange;">双峰</span>的分界线
...
则<span style="border-bottom: 1.5px dashed orange;">经过信道</span>后
...
=== "误码率"
    ...
    - 在<span style="border-bottom: 1.5px dashed orange;">误码率</span>一定下，所需 SNR 反之，即 $\mathbf{r}_{\mathrm{2ASK}}=2\mathbf{r}_{\mathrm{2FSK}}=4\mathbf{r}_{\mathrm{2PSK}}$
