...
存在<span style="border-bottom: 1.5px dashed orange;">乘性干扰</span>(信道引入)与加性噪声
...
- 随机信道：错码独立出现
- 突发信道：脉冲信号引发的<span style="border-bottom: 1.5px dashed orange;">集中错码</span>
- 混合信道：混合随机与突发信道
...
- 码距：两 <span style="border-bottom: 1.5px dashed orange;">许用码组</span> 对应位不同的个数(异或求和),称为【汉明距离】，用 `d` 表示
- 最小码距$d_{0}$：所有 <span style="border-bottom: 1.5px dashed orange;">许用码组</span> 之间码距的最小值  
  <span style="border-bottom: 1.5px dashed orange;">对于线性码，最小码距等于非全零码的最小重量，即 </span>$d_{0}=w_{0}$
...
??? tip "题外话—比特信噪比 Eb/N0"
    ...
    - SNR(Signal-Noise Ratio)即信噪比，在<span style="border-bottom: 1.5px dashed orange;">模拟通信系统</span>中，是接收端模拟信号的重要测量指标，可以通过频谱仪等仪器直接实际测量接收端的模拟信号得到
    - Eb/N0 则是指通信系统中<span style="border-bottom: 1.5px dashed orange;">传输一比特信息</span>所需要的<span style="border-bottom: 1.5px dashed orange;">能量和噪声功率谱密度</span>的比值，是衡量整个通信系统性能归一化的一个性能指标
...
按照一组<span style="border-bottom: 1.5px dashed orange;">线性方程</span>构成的<span style="border-bottom: 1.5px dashed orange;">分组码</span>

<span style="border-bottom: 1.5px dashed orange;">线性码</span>：督元是信元的线性组合

<span style="border-bottom: 1.5px dashed orange;">校正子</span> $S=a_{n-1}\oplus a_{n-2}\oplus\cdots\oplus a_{0}$
...
取等号即为<span style="border-bottom: 1.5px dashed orange;">汉明码</span>，即 $\boxed{(n,k)=(2^{r}-1,2^{r}-1-r)}$
...
> 为 `(r·n)` 的矩阵，<span style="border-bottom: 1.5px dashed orange;">每一列</span>为错码位置对应的<span style="border-bottom: 1.5px dashed orange;">监督表达式</span>
...
对于<span style="border-bottom: 1.5px dashed orange;">典型监督矩阵 H</span>，可写作
...
此即为<span style="border-bottom: 1.5px dashed orange;">典型生成矩阵</span>（左边为单位阵 `I`）
...
!!! tip "你不得不知道的"
    ...
    我们知道其是由 <span style="border-bottom: 1.5px dashed orange;">单位阵（用于表示信息阵码组）与对应的监督关系式</span> 组成的
    ...
    **而把思维逆转过来，这是不是表明，当已知上面的许用码组时，能直接反向解析出生成矩阵呢？**
    ...
    2. 非典型矩阵可通过<span style="border-bottom: 1.5px dashed orange;">行列变换</span>变成典型矩阵
...
许用码组模 2 加<span style="border-bottom: 1.5px dashed orange;">一定还在许用码组的范围</span>，土想想也行，若要证明有
...
> 零点是全零码组，那也可以推出 <span style="border-bottom: 1.5px dashed orange;">最小码重对应最小码距</span> 的结论
...
用码多项式表达<span style="border-bottom: 1.5px dashed orange;">循环码</span>，即
...
!!! success "结论"
    ...
    一个长为 `n` 的循环码的码多项式 都是按模 $(x^{n}+1)$ 运算的<span style="border-bottom: 1.5px dashed orange;">一个余式</span>
...
!!! quote "以下所述的码多项式是【次数】而非【维数】（高项为 0），不要混淆了"
    ...
    基于<span style="border-bottom: 1.5px dashed orange;">生成矩阵每一行都是许用码组</span>的特性，叠加循环码的<span style="border-bottom: 1.5px dashed orange;">左移亦许用</span>特征
...
!!! tip "意外发现"
    ...
    <span style="border-bottom: 1.5px dashed orange;">最低位必须为 1</span>，否则 `g(x)` 右移后，信元全为 0，但督元最高位为 1，不符合逻辑
...
你将在<span style="border-bottom: 1.5px dashed orange;">信息论与编码</span>与他们再会
