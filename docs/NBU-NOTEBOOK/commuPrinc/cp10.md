...
不同前提条件下的<span style="border-bottom: 1.5px dashed orange;">最佳接收</span>定义不同
...
以二进制为例 （回忆<span style="border-bottom: 1.5px dashed orange;">二进制基带抗噪性能分析</span>）

### 噪声 n(t)的统计特性

设 $n(t)$ 是零均值的高斯白噪声

则通过随机信号分析的知识，得到其<span style="border-bottom: 1.5px dashed orange;">K维概率密度函数为边缘概率密度函数的乘积</span>，即
...
???+ info "这里的概率密度函数是离散的，而我们希望得到连续的过程 $n(t)$"
    ...
    <span style="border-bottom: 1.5px dashed orange;">当 K 很大很大时</span>，噪声在一个 $T_{B}$ 内的平均功率可表示为

    $$
    {\frac{1}{k}}\sum_{i=1}^{k}n_{i}^{2}={\frac{1}{T_{\mathrm{B}}2f_{H}}}\sum_{i=1}^{k}n_{i}^{2}
    $$

    高数中已知函数积分的离散近似（这里的平方表示是<span style="border-bottom: 1.5px dashed orange;">功率</span>）
    ...
    又因为<span style="border-bottom: 1.5px dashed orange;">零均值下均方值（功率）与方差相等</span>，即有 $\sigma_{n}^{2}=n_{0}f_{H}$ ，则可将上式整理为
...
用<span style="border-bottom: 1.5px dashed orange;">似然函数</span>判决 0/1（等概下）
...
即<span style="border-bottom: 1.5px dashed orange;">相关接收机（以二进制为例）</span>
...
!!! success "结论"
    ...
    最佳接收结构为<span style="border-bottom: 1.5px dashed orange;">r(t)与码元波形混频后积分再比较判决</span>

!!! info ""

    ![](https://cdn.jsdelivr.net/gh/dixiLOG/blogStatic/DBO5blzcMoSdxvxWMD5czVLZnnc.png)

    > 关于相关接收机，需要注意积分器 buffer 每次运算后需要清零，且对符号定时<span style="border-bottom: 1.5px dashed orange;">同步</span>讲究
...
随相，即随机相位（初始相位随机，而<span style="border-bottom: 1.5px dashed orange;">码元内信号连续</span>）
...
得到<span style="border-bottom: 1.5px dashed orange;">随相信号</span>的【最佳接收判决规则】
...
匹配滤波器：使得在<span style="border-bottom: 1.5px dashed orange;">抽样时刻</span>上获得<span style="border-bottom: 1.5px dashed orange;">最大 SNR</span>的<span style="border-bottom: 1.5px dashed orange;">线性</span>滤波器,常在发送和接收端放置参数匹配的 RRC，做 pulse shape

> 本节讨论均在<span style="border-bottom: 1.5px dashed orange;">高斯白噪声</span>的前提下
...
至于$t_{0}$ 到底是啥时候取决于<span style="border-bottom: 1.5px dashed orange;">符号定时同步模块(Symbol Timing Synchronisation)</span>
...
只要<span style="border-bottom: 1.5px dashed orange;">信号能量 E 和噪声功率谱密度 n0</span>确定，匹配滤波接收法适用于<span style="border-bottom: 1.5px dashed orange;">任何</span>数字信号波形
